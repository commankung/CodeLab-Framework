...
{
	$ss = "SELECT * FROM TBL_TRAINING WHERE CID='$CourseID' ";
	$qs = mssql_query($ss);
	$rs= mssql_fetch_array($qs);
	$courseName = $rs['TITLE'];
	
	
	$d1=str_replace(' ','',$rs['TDATE']);
	$d2=explode('-',$d1);
	//$d3=DateTime::createFromFormat('m/d/Y', $d2[0]);
	if($d2[0]==$d2[1])
	{
		$t_date=date("d/m/Y", strtotime($d2[0]));
		$t_time=$rs['S_TIME']."-".$rs['E_TIME'];			
	}else
	{
	...
	if(iconv("tis-620", "utf-8",$rs['BRANCH'])=="OTHER")
	{
		if(iconv("tis-620", "utf-8",$rs['Other'])=="-")
		{
			$t_location=iconv("tis-620", "utf-8",$rs['ROOM']);
		}else{
			$t_location=iconv("tis-620", "utf-8",$rs['ROOM'])." @ ".$rs['Other'];				
		}
	}else{
		$t_location=iconv("tis-620", "utf-8",$rs['ROOM'])." @ ".iconv("tis-620", "utf-8",$rs['BRANCH']);
	}

	
	$nquota=intval($rs['N_NUMBER'])+intval($rs['S_NUMBER']);

	$scount=" SELECT COUNT(*) as cquota FROM TBL_TRAINING_REGISTER where CourseID='".$CourseID."'";
	...
	if(intval($rcount['cquota'])==$nquota)
	{
		...
		$sra = "SELECT * FROM TBL_TRAINING_REGISTER where CourseID='".$CourseID."' and EmpID='".$EmpID."'";
		$qra = mssql_query($sra);
		$rgsc=intval(mssql_num_rows($qra));
		if($rgsc==0)
		{
			$sql = "insert into TBL_TRAINING_REGISTER(EmpID,Name_TH,Name_EN,Position,Department,Company,Mobile,CourseID,Reg_Date,Reg_Status,Email_Mrg,Point_Car,Title_Name,Reg_Time,IP)values('$EmpID','$NameTh','$NameEn','$Position_Emp','$Department_Emp','$AGR_COMPANY','$Mobile_Emp','$CourseID','".date('m-d-Y')."','Waiting','$emrg','$Point_Car','$Title_name','".date('H:i:s')."','$ipc')";
			mssql_query($sql);
			...
			{
				$sql_update = "update TBL_TRAINING_PROFILE_USER  set Mobile='$Mobile_Emp',Email_Mrg='$emrg',Point_Car='$Point_Car' where EmpID='".$EmpID."'";
...
if( $statusSubmit=="Edit")
{
			...
			if($rgscx1>0)
			{
				...
				$sqlREGISTER = "SELECT * FROM TBL_TRAINING_REGISTER where EmpID='".$EmpID."' AND CourseID='".$CourseID."'"; 
				$qREGISTER = mssql_query($sqlREGISTER);
				$rowREGISTER=intval(mssql_num_rows($qREGISTER));			
				if($rowREGISTER>0)
				{
					$sqlUpdateReg = "update TBL_TRAINING_REGISTER  set EmpID='$EmpID',Name_TH='$NameTh',Name_EN='$NameEn',Position='$Position_Emp',Department='$Department_Emp',Company='$AGR_COMPANY',Mobile='$Mobile_Emp',Email_Mrg='$emrg',Point_Car='$Point_Car',Title_Name='$Title_name' where CourseID='$CourseID' AND EmpID='$EmpID'";
